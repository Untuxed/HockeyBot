    ...
    if str(after.author) == 'sesh#1244':
        ...
        for j, gameTime in enumerate(Current_Scheduled_Games):
            if legibleDateTime == gameTime:
                ...
                await cellOperations.Update_Range(Confirmed_Player_Range, Confirmed_Player_Values)
                await cellOperations.Update_Range(Maybe_Player_Range, Maybe_Player_Values)
                await cellOperations.Update_Range(Scratched_Player_Range, Scratched_Player_Values)
                return
