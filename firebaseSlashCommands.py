...
# from googleStuff import firestore
from firebaseStuff import *
from genericFunctions import checkDuplicatePlayer, get_player, generate_stats_message

db = firestore.client()


# region get-my-stats: pulls your stats from the google sheet
@tree.command(name='get-my-stats-firebase', description='Views a players stats', guild=GUILD_ID)
async def getMyStats(interaction: discord.Interaction):
    match = re.match(r'(\w+)\s+(\w+)\s+\[(\d+)\]', interaction.user.nick)
    first_name, last_name, number = match.groups()
    playerID = f'{first_name}_{last_name}_{number}'
    player = await get_player(playerID)
    stats_message = generate_stats_message(player, interaction.user.nick)
...
# region add-player
@tree.command(name='add-player-firebase', description='Adds a Player to the Firestore db', guild=GUILD_ID)
